...
export default function Portfolio() {
  ...
  // Sort newest → oldest
  const sortByYearDesc = (a: Artwork, b: Artwork) => Number(b.year) - Number(a.year);

  const schoolPeriod = artworks.filter(a => a.period === "school").sort(sortByYearDesc);
  const gamesSeries = artworks.filter(a => a.period === "games").sort(sortByYearDesc);
  const hypoCrisisSeries = artworks.filter(a => a.period === "hypo-crisis").sort(sortByYearDesc);
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="flex justify-center mb-12">
          <a
            ...
            download
            className="inline-block bg-primary text-white font-medium px-6 py-2 rounded-md shadow hover:bg-primary/90 transition-colors"
          >
            Download Portfolio (PDF)
          </a>
        </div>

        {/* THE EARTH ERASES EVERYTHING EXCEPT THE PROMISE */}
        <section className="mb-24">
          <div className="bg-muted/30 py-6 px-8 rounded-md">
            <h2 className="font-serif text-3xl md:text-4xl font-light">
              {t("portfolio.promise.title")} <br />({t("portfolio.promise.date")})
            </h2>
            <p className="text-muted-foreground mt-1">
              {t("portfolio.promise.location")}
            </p>
            <div className="text-muted-foreground mt-6 max-w-4xl leading-relaxed text-justify space-y-4">
              <p>
                {t("portfolio.promise.desc")}
              </p>
              <p>
                {t("portfolio.promise.support")}
              </p>
              <p>
                {t("portfolio.promise.cc4wbs")}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="mb-24">
          <div className="bg-muted/30 py-6 px-8 rounded-md mb-12">
            <h2 className="font-serif text-3xl md:text-4xl font-light">Hypo-Crisis Series (2020–2024)</h2>
            <p className="text-muted-foreground mt-3 max-w-4xl leading-relaxed text-justify whitespace-pre-line">
        ...
        <section className="mb-24">
          <div className="bg-muted/30 py-6 px-8 rounded-md mb-12">
            <h2 className="font-serif text-3xl md:text-4xl font-light">Games Series (2019)</h2>
        ...
        <section>
          <div className="bg-muted/30 py-6 px-8 rounded-md mb-12">
            <h2 className="font-serif text-3xl md:text-4xl font-light">School Period (2012–2018)</h2>
